...
const vec4 TRAIL_COLOR = vec4(0.2, 0.6, 1.0, 1.0); // blue
const vec4 CURRENT_CURSOR_COLOR = TRAIL_COLOR;
const vec4 PREVIOUS_CURSOR_COLOR = TRAIL_COLOR;
const vec4 TRAIL_COLOR_ACCENT = vec4(0.0, 0.85, 1.0, 1.0); // cyan accent
// Longer and held j/k retriggers before it finishes, which reads as lag
const float DURATION = .15;
const float OPACITY = .2;
// In cursor sizes; below 3.0, coalesced j/k moves make the trail strobe
const float DRAW_THRESHOLD = 3.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (isFarEnough && isOnSeparateLine) {
        ...
        float sdfCursor =
            getSdfRectangle(vu, currentCursor.xy - (currentCursor.zw * offsetFactor), currentCursor.zw * 0.5);
